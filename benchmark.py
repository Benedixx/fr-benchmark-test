...
def prepare_db_embeddings_facenet(model):
    local_known_faces = {}
...
def preprocessing_facenet(image):
    faces = detector.detectMultiScale(image, 1.1, 4)
...
def benchmark_accuracy(model, model_name):
    ...
    for test_image, true_label in zip(test_images, label_images):
        ...
        if detection_list:
            ...
            print(f"Detected person: {detected_person}, True label: {true_label}")

            if "Novebri Tito Ramadhani" in detected_person:
...
def benchmark_accuracy_facenet(model, model_name):
    known_faces = prepare_db_embeddings_facenet(model)
    test_images, label_images = prepare_test_set_facenet()
    total_images = len(test_images)
    correct_predictions = 0
...
benchmark_accuracy_facenet(facenet_model, 'FaceNet')
